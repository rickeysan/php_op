...
class Patient extends Human{
    ...
    function judgeCondition($flg){
        ...
        if($this->getConditionFlg() == 0 && $this->hp >= 100){
            $msg.='が回復しました';
            History::set($msg);
...
if($doctor_action_flg){
    debug('医者が行動します');
    if(isset($_POST['doctor_action1']) && $_POST['doctor_action1'] != 4){
        debug('ドクター1治療');
        $standard_point = $_SESSION['doctors'][0]->getSkill()*10;
        $treatment_point = mt_rand($standard_point,$standard_point+20);
        debug('ドクター1の治療ポイント'.$treatment_point);
        $_SESSION['doctors'][0]->treatment(decideTreatment($_POST['doctor_action1']),$treatment_point);
    }elseif(isset($_POST['doctor_action1']) && $_POST['doctor_action1'] == 4){
        debug('ドクター1研修');
        $_SESSION['doctors'][0]->setSkill($_SESSION['doctors'][0]->getSkill()+1);
    }


    if(isset($_POST['doctor_action2']) && $_POST['doctor_action2'] != 4){
        debug('ドクター2治療');
        $standard_point = $_SESSION['doctors'][1]->getSkill()*10;
        debug('ドクター2の基準ポイント'.$standard_point);
        $treatment_point = mt_rand($standard_point,$standard_point+20);
        debug('ドクター2の基準ポイント'.$treatment_point);
        $_SESSION['doctors'][1]->treatment(decideTreatment($_POST['doctor_action2']),$treatment_point);
    }elseif(isset($_POST['doctor_action2']) && $_POST['doctor_action2'] == 4){
        debug('ドクター2研修');
        $_SESSION['doctors'][1]->setSkill($_SESSION['doctors'][1]->getSkill()+1);
    }
    updatePatients($_SESSION['milds'],false);
    updatePatients($_SESSION['moderates'],false);
    updatePatients($_SESSION['criticallys'],false);
    ...
    updatePatients($_SESSION['milds'],true);
    updatePatients($_SESSION['moderates'],true);
    updatePatients($_SESSION['criticallys'],true);
